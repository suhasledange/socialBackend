...
const updateUserAvatar = asyncHandler(async (req,res)=>{
  ...
  if(!avatarLocalPath) new ApiError(400,"Avatar file is missing")
    ...
    if(!avatar.url) new ApiError(400,"Error while uploading avatar")
...
const updateUserCover = asyncHandler(async (req,res)=>{
  ...
  if(!coverImageLocalPath) new ApiError(400,"Cover image file is missing")
    ...
    if(!coverImage.url) new ApiError(400,"Error while uploading cover image")
...
const getUserChannelProfile = asyncHandler(async (req,res)=>{
  ...
  const channel = await User.aggregate([
    ...
    //based on one document lookup for and find subcriber
    {
      $lookup:{
        from:"subscriptions",
        localField:"_id",
